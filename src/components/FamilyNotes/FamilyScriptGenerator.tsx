...
${currentUser?.fullName}`;
  ...
  const downloadAsPDF = () => {
    // Create downloadable text file for demo
    const content = `Family Communication Script
Generated by Lumi

Child: ${scriptData.childName}
Parent: ${scriptData.parentName}
Language: ${language}
...
${generatedScript}`;
    ...
    link.href = url;
    link.download = `family_script_${scriptData.childName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.txt`;
