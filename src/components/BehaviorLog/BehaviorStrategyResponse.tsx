...
export const BehaviorStrategyResponse: React.FC<BehaviorStrategyResponseProps> = ({
  ...
  React.useEffect(() => {
    if (child && behaviorLog) {
      autoGenerateFamilyScript(child, behaviorLog, 'english')
        .then(script => setAutoGeneratedScript(script))
        .catch(error => console.error('Error generating family script:', error));
    }
  }, [child, behaviorLog]);
  const strategies = [
    {
  ...
  const handleConfirm = () => {
    if (selectedStrategy) {
      const strategy = response.practicalStrategies?.[parseInt(selectedStrategy.replace('strategy-', ''))] || '';
      onStrategySelect(strategy, confidence);
    }
  };
        ...
        <div className="space-y-8">
          ...
          {autoGeneratedScript && (
            <Card className="p-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex space-x-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => navigator.clipboard.writeText(autoGeneratedScript)}
                  >
                    Copy Script
                  </Button>
                  <Button
              ...
              <div className="bg-white border border-[#E6E2DD] rounded-xl p-6 max-h-64 overflow-y-auto">
                <div className="whitespace-pre-line text-gray-700 leading-relaxed text-sm">
                  {autoGeneratedScript.substring(0, 500)}...
                </div>
              </div>
            </Card>
          )}
          ...
          {selectedStrategy && (
            <Card className="p-8 bg-[#F8F6F4]">
              ...
              <div className="space-y-8">
                ...
                <div>
                  <h4 className="font-medium text-[#1A1A1A] mb-4">
                    How doable is this strategy in your classroom context?
                  </h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full h-2 bg-[#E6E2DD] rounded-lg appearance-none cursor-pointer"
                        style={{
                          background: `linear-gradient(to right, #10B981 0%, #10B981 ${doabilityRating * 10}%, #E6E2DD ${doabilityRating * 10}%, #E6E2DD 100%)`
                        }}
                      />
                      <div className="flex justify-between mt-2">
                        {Array.from({length: 10}).map((_, i) => (
                          <div key={i} className="flex flex-col items-center">
                            <div className={`w-2 h-2 rounded-full ${i < doabilityRating ? 'bg-green-500' : 'bg-[#E6E2DD]'}`} />
                            <span className="text-xs text-gray-500 mt-1">{i + 1}</span>
                          </div>
                        ))}
                      </div>
